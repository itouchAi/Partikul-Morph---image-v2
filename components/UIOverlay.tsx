            ...
            <div className="relative flex flex-col items-center"> 
                ...
                <div className={`absolute top-full flex flex-col gap-1 items-center w-10 pt-1 ${isShapeMenuOpen ? 'shape-menu-open' : ''}`}> 
                    ...
                    <div className="theme-menu-item item-1 relative flex items-center justify-end w-full group/sphere pr-1">
                        ...
                        <button onClick={() => handleShapeSelect('sphere')} className="w-10 h-10 rounded-full border border-white/20 flex items-center justify-center transition-all duration-300 bg-black/60 backdrop-blur text-white hover:scale-110 hover:bg-cyan-600/50 hover:border-cyan-400 hover:shadow-[0_0_15px_rgba(34,211,238,0.5)] z-10 animate-in zoom-in duration-300" title="Küre">
                    ...
                    <div className="theme-menu-item item-2 relative flex items-center justify-end w-full group/cube pr-1">
                        <button disabled={!hasImage} className={`absolute right-full mr-1 w-8 h-8 rounded-full border border-white/20 flex items-center justify-center transition-all duration-300 ${hasImage ? 'bg-gradient-to-br from-pink-500 via-purple-500 to-indigo-500 text-white shadow-[0_0_15px_rgba(168,85,247,0.6)] ring-1 ring-white/50 scale-100 opacity-100 group-hover/cube:scale-110' : 'bg-gray-900/80 text-gray-600 cursor-not-allowed border-gray-700 opacity-0'} group-hover/cube:opacity-100 group-hover/cube:delay-0 delay-150 translate-x-4 group-hover/cube:translate-x-0 pointer-events-none group-hover/cube:pointer-events-auto`} title={hasImage ? "Resim Dokusunu Uygula (Yakında)" : "Önce Resim Yükleyin"}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`${hasImage ? 'animate-bounce' : ''}`}><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>
                        </button>
                        <button onClick={() => handleShapeSelect('cube')} className="w-10 h-10 rounded-full border border-white/20 flex items-center justify-center transition-all duration-300 bg-black/60 backdrop-blur text-white hover:scale-110 hover:bg-purple-600/50 hover:border-purple-400 hover:shadow-[0_0_15px_rgba(192,132,252,0.5)] z-10 animate-in zoom-in duration-300" title="Küp">
