  ...
  return (
    <>
      <style>{`
        ...
        .stylish-scrollbar::-webkit-scrollbar-track { background: transparent; margin: 4px 0; }
        .stylish-scrollbar::-webkit-scrollbar-thumb { background: rgba(156, 163, 175, 0.3); border-radius: 20px; transition: background 0.3s; }
        .stylish-scrollbar:hover::-webkit-scrollbar-thumb { background: rgba(156, 163, 175, 0.6); }
            ...
            <div className="relative flex flex-col items-center"> 
                ...
                <div className={`absolute top-full flex flex-col gap-1 items-center w-10 pt-1 ${isShapeMenuOpen ? 'shape-menu-open' : ''}`}> 
                    ...
                    <div className="theme-menu-item item-1 relative flex items-center justify-center w-full group/sphere mb-1">
                        {/* Doku Butonu (Texture) */}
                        <button disabled={!hasImage} className={`absolute right-full mr-2 w-10 h-10 rounded-full border border-white/20 flex items-center justify-center transition-all duration-300 ${hasImage ? 'bg-cyan-600/20 text-cyan-300 border-cyan-500/50 shadow-[0_0_15px_rgba(34,211,238,0.6)] hover:bg-cyan-600/40 hover:scale-110 animate-pulse' : 'bg-gray-900/80 text-gray-600 cursor-not-allowed border-gray-700 opacity-0'} group-hover/sphere:opacity-100 group-hover/sphere:delay-0 delay-150 translate-x-4 group-hover/sphere:translate-x-0 pointer-events-none group-hover/sphere:pointer-events-auto`} title={hasImage ? "Resim Dokusunu Uygula" : "Önce Resim Yükleyin"}>
                    ...
                    <div className="theme-menu-item item-2 relative flex items-center justify-center w-full group/cube mb-1">
                        {/* Doku Butonu (Texture) */}
                        <button disabled={!hasImage} className={`absolute right-full mr-2 w-10 h-10 rounded-full border border-white/20 flex items-center justify-center transition-all duration-300 ${hasImage ? 'bg-purple-600/20 text-purple-300 border-purple-500/50 shadow-[0_0_15px_rgba(168,85,247,0.6)] hover:bg-purple-600/40 hover:scale-110 animate-pulse' : 'bg-gray-900/80 text-gray-600 cursor-not-allowed border-gray-700 opacity-0'} group-hover/cube:opacity-100 group-hover/cube:delay-0 delay-150 translate-x-4 group-hover/cube:translate-x-0 pointer-events-none group-hover/cube:pointer-events-auto`} title={hasImage ? "Resim Dokusunu Uygula" : "Önce Resim Yükleyin"}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`${hasImage ? 'animate-bounce' : ''}`}><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>
                        </button>
                        {/* Ana Küp Butonu */}
